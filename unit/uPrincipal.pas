...
type
  TfrmPrincipal = class(TForm)
    ...
    procedure btnCadastraTipoClick(Sender: TObject);
  private
...
uses uDM, uGeral, uCadastraCont, uCadastraCompromisso, uLogin, ZDataset, uInfosComp,
  uCadastraBase, uConfig, StrUtils, uTrocaSenha, uCadTipoComp;
...
procedure TfrmPrincipal.btnCadastraTipoClick(Sender: TObject);
begin
  if (frmCadTipoComp = nil) then
    Application.CreateForm(TfrmCadTipoComp,frmCadTipoComp);
  frmCadTipoComp.SetarTitulo('Cadastrar tipo de compromisso');
  dm.conn.Commit;
  ShowMessage(frmCadTipoComp.zqryTipoComp.SQL.Text);
  frmCadTipoComp.ShowModal;
end;
