...
-- Copiando dados para a tabela agenda.cidade: ~5.564 rows (aproximadamente)
/*!40000 ALTER TABLE `cidade` DISABLE KEYS */;
INSERT IGNORE INTO `cidade` (`id`, `nome`, `estado`) VALUES
	(1, 'Afonso Cláudio', 8),
...
-- Copiando dados para a tabela agenda.estado: ~27 rows (aproximadamente)
...
DROP TABLE IF EXISTS `tb_compromissos`;
...
CREATE TABLE IF NOT EXISTS `tb_login` (
  ...
  `dt_cadastro` datetime DEFAULT NULL,
  `dt_atualizacao` datetime DEFAULT NULL,
  `ds_login` varchar(50) NOT NULL DEFAULT '0',
  `ds_senha` varchar(50) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `tb_compromissos_v` (
	`contato` VARCHAR(50) NULL COLLATE 'utf8mb4_0900_ai_ci',
	`datacomp` DATE NULL,
	`horacomp` TIME NULL,
	`desccomp` VARCHAR(1000) NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;

-- Copiando estrutura para view agenda.tb_contatos_v
DROP VIEW IF EXISTS `tb_contatos_v`;
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `tb_contatos_v` (
	`NOME` VARCHAR(50) NULL COLLATE 'utf8mb4_0900_ai_ci',
	`ENDERECO` VARCHAR(50) NULL COLLATE 'utf8mb4_0900_ai_ci',
...
DROP PROCEDURE IF EXISTS `insereLogin`;
DELIMITER //
CREATE PROCEDURE `insereLogin`(
    ...
    DETERMINISTIC
BEGIN
	INSERT INTO tb_login (ds_login,ds_senha) VALUES (UPPER(p_login), UPPER(SHA1(MD5(p_senha))));
END//
DELIMITER ;
...
DELIMITER //
CREATE FUNCTION `retornaNomeCidade`(
	`id_estado_p` INT,
	`id_cidade_p` INT
) RETURNS text CHARSET utf8mb4
    DETERMINISTIC
BEGIN
	DECLARE NOMECIDADE VARCHAR(120) DEFAULT NULL;
	SELECT A.NOME
	INTO NOMECIDADE
	FROM cidade AS A
	WHERE A.ID = id_cidade_p
	AND A.estado = id_estado_p;
	RETURN NOMECIDADE;
END//
DELIMITER ;

-- Copiando estrutura para função agenda.retornaNomeEstado
DROP FUNCTION IF EXISTS `retornaNomeEstado`;
DELIMITER //
CREATE FUNCTION `retornaNomeEstado`(
	`id_estado_p` INT
) RETURNS text CHARSET utf8mb4
    DETERMINISTIC
BEGIN
	DECLARE NOMEEST VARCHAR(75) DEFAULT NULL;
	SELECT NOME
	INTO NOMEEST
	FROM estado
	WHERE ID = id_estado_p;
	RETURN NOMEEST;
END//
DELIMITER ;

-- Copiando estrutura para trigger agenda.tb_compromissos_atualizacao
DROP TRIGGER IF EXISTS `tb_compromissos_atualizacao`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tb_compromissos_atualizacao` BEFORE UPDATE ON `tb_compromissos` FOR EACH ROW BEGIN
	SET NEW.dt_atualizacao := NOW();
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Copiando estrutura para trigger agenda.tb_compromissos_cadastro
DROP TRIGGER IF EXISTS `tb_compromissos_cadastro`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
...
-- Copiando estrutura para trigger agenda.tb_contatos_atualizacao
DROP TRIGGER IF EXISTS `tb_contatos_atualizacao`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tb_contatos_atualizacao` BEFORE UPDATE ON `tb_contatos` FOR EACH ROW BEGIN
	SET NEW.dt_atualizacao := NOW();
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Copiando estrutura para trigger agenda.tb_contatos_cadastro
DROP TRIGGER IF EXISTS `tb_contatos_cadastro`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tb_contatos_cadastro` BEFORE INSERT ON `tb_contatos` FOR EACH ROW BEGIN
	SET NEW.dt_cadastro := NOW();
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Copiando estrutura para trigger agenda.tb_login_atualizacao
DROP TRIGGER IF EXISTS `tb_login_atualizacao`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tb_login_atualizacao` BEFORE UPDATE ON `tb_login` FOR EACH ROW BEGIN
	SET NEW.dt_atualizacao := NOW();
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Copiando estrutura para trigger agenda.tb_login_cadastro
DROP TRIGGER IF EXISTS `tb_login_cadastro`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tb_login_cadastro` BEFORE INSERT ON `tb_login` FOR EACH ROW BEGIN
	SET NEW.dt_cadastro := NOW();
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
...
DROP TABLE IF EXISTS `tb_compromissos_v`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `tb_compromissos_v` AS select `b`.`nome` AS `contato`,`a`.`dt_comp` AS `datacomp`,`a`.`hr_comp` AS `horacomp`,`a`.`desc_comp` AS `desccomp` from (`tb_compromissos` `a` join `tb_contatos` `b` on((`b`.`id` = `a`.`id_contato`)));
